...
const toLastNames = people => {
  const fullNames = people.map((obj) => `${obj.firstName} ${obj.lastName}`);
  return fullNames;
};
...
const addValues = (arr) => {
  const sumOfValues = arr.reduce( (accumulator, value, idx) => {
...
const addPurchases = (arr) => {
  const purchaseTotal = arr.reduce( (accumulator, value, idx) => {
