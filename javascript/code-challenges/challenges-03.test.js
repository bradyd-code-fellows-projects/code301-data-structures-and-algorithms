...
const typeNum = (arr) => {
  let numsOnly = arr.filter((val) => {return typeof (val) !== 'string';});
  return numsOnly;
};
...
const containsAnd = (arr) => {
  let stringHasAnd = arr.filter( val => {return val.includes('and');});
  return stringHasAnd;
};
...
const oddValues = (arr) => {
  let oddsOnly = arr.filter( val => {return !(val % 2 === 0);});
  return oddsOnly;
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  let notInFirst = arr.filter( val => {return !(forbiddenValues.includes(val));});
  return notInFirst;
};
...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  let baseStatAboveX = arr.filter( val => {
    if ((snorlaxData.stats[val].baseStat) > minBaseStat) {
      return snorlaxData.stats[val];
    }
  });
  return baseStatAboveX;
};
